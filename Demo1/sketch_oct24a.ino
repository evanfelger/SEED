...
double r = 0.0678; // Radius of the wheel
...
void loop() {
  ...
  if (millis()-previousMillisRIGHT>10){
    thetaDotRIGHT = 0;
  }
  if (millis()-previousMillisLEFT>10){
    thetaDotLEFT = 0;
  }
  Serial.print(Va1/8*150);
  Serial.print(" ");
  Serial.print(currentPhiDot);
  Serial.print(" ");
  Serial.println(errorRho);
  analogWrite(9,abs(round(Va1/8*180)));
  analogWrite(10,abs(round(Va2/8*181.2)));
}
...
void turn(double angle){
  ...
  if((round(errorPhi*1000000)==0.00)){
    IPhi = 0;
  }
